				...
				<?php if(has_post_thumbnail() && $images == "true") { ?>
					...
					<div class="post-thumbnail<?php if($image_wrap == "true") { ?> wrap<?php } ?>">
						...
						<?php } ?>
							...
							<?php $image = aq_resize(wp_get_attachment_url(get_post_thumbnail_id($post->ID)), $thumbnail_width, $thumbnail_height, $hard_crop, false, true); ?>
							<?php if(get_option($dirname.'_retina') == "0") { $retina = aq_resize(wp_get_attachment_url(get_post_thumbnail_id($post->ID)), $thumbnail_width*2, $thumbnail_height*2, $hard_crop, false, true); } else { $retina = ""; } ?>
