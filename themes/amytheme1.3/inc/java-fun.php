...
<?php
global $ab_amy_settings ,$ab_amy_bgslideshow ,$is_404;
if($is_404 == true){
	$id = $ab_amy_settings['404-page'];
	$post_meta_data = get_post_custom($ab_amy_settings['404-page']);
}else{
	$id = get_the_ID();
...
$ab_tf_body_color_scheme = $ab_amy_settings['body-color-scheme'];
...
<style>
<?php echo $ab_amy_settings['add-custom-css'];?>
...
<?php if ($ab_amy_settings['menu-background-shadow'] == false){?>
.header-white {
...
<?php }?>
<?php if ($ab_amy_settings['menu-background-gradient'] == true){?>
.header-white {
 	background: -webkit-linear-gradient(<?php echo $ab_amy_settings['menu-background']['color'];?>, transparent);
    background: -o-linear-gradient(<?php echo $ab_amy_settings['menu-background']['color'];?>, transparent); 
    background: -moz-linear-gradient(<?php echo $ab_amy_settings['menu-background']['color'];?>, transparent); 
    background: linear-gradient(<?php echo $ab_amy_settings['menu-background']['color'];?>, rgba(255,255,255,0)); 
	height:<?php echo $ab_amy_settings['menu-background-gradient-height'];?>px;
	
	opacity:<?php echo $ab_amy_settings['menu-background']['alpha'];?>
}
<?php }?>
...
#customer_details ::-webkit-input-placeholder {
  color:<?php echo $ab_amy_settings['woo-input-color']['color']?>!important;
}
#mainmenu .dl-menuwrapper button:after{
	background: <?php echo $ab_amy_settings['menu-typography']['color'];?>;
	box-shadow: 0 7px 0 <?php echo $ab_amy_settings['menu-typography']['color'];?>, 0 14px 0 <?php echo $ab_amy_settings['menu-typography']['color'];?>;
}
...
?>
<script>
<?php echo $ab_amy_settings['add-custom-js'];?>
jQuery(document).ready(function($){
	/*if($('#mainmenu div').hasClass('mega-menu-wrap')){
		$(".responsivemenuwarp").addClass("hideelement");
		$(".dl-menuwrapper").addClass("hideelement");
  ...
  .mouseout(function() {
	...
	if($('body').hasClass('vc_editor')){
		$(".ss-stand-alone").removeClass("fullwidthrow");
	}
...
for(var i = 0; i < Opentip.tips.length; i ++) { Opentip.tips[i].hide(); }
	...
	if(!$("#mainpage div").hasClass('wpb_row') && !$('body').hasClass('vc_editor')  ){
  ...
  <?php if($ab_amy_settings['footer-position'] != "absolute"){?>
		if($(window).width() > 810){
	...
	$(this).click(function(event) {
		...
		var elClasses = $(this).attr('href').replace(/^.*?#/,'');
				if($('.header-top-p div').hasClass(elClasses)){
					...
					$('html,body').animate({
					<?php if($ab_amy_settings['menu-position'] == "absolute"){?>
...
//	$("a[rel^='prettyPhoto']").prettyPhoto({theme: 'dark_square', allow_resize: false,});
	...
	function update(widget_id, time_format, date_format) {
		var months = new Array("<?php echo $ab_amy_settings['tr-months-jan'];?>", "<?php echo $ab_amy_settings['tr-months-feb'];?>", "<?php echo $ab_amy_settings['tr-months-mar'];?>", "<?php echo $ab_amy_settings['tr-months-apr'];?>", "<?php echo $ab_amy_settings['tr-months-may'];?>", "<?php echo $ab_amy_settings['tr-months-jun'];?>", "<?php echo $ab_amy_settings['tr-months-jul'];?>", "<?php echo $ab_amy_settings['tr-months-aug'];?>", "<?php echo $ab_amy_settings['tr-months-sep'];?>", "<?php echo $ab_amy_settings['tr-months-oct'];?>", "<?php echo $ab_amy_settings['tr-months-nov'];?>", "<?php echo $ab_amy_settings['tr-months-dec'];?>"),
		ampm = " AM",
		now = new Date(),
		hours = now.getHours(),
...
<?php
if($ab_amy_settings['yt-active'] == true && $ab_amy_bgslideshow == 1 || $ab_amy_settings['yt-active'] == true && $ab_amy_settings['site-width'] == "boxed"){?>
		<script>
		//Youtube API
		//==================================================
        jQuery( document ).ready( function($){
             $('#ytbgvideo').tubular({videoId: '<?php echo $ab_amy_settings['yt-bg-id']; ?>', mute: <?php echo $ab_amy_settings['yt-bg-mute'];?> , start: '<?php echo $ab_amy_settings['yt-bg-start'];?>', repeat:<?php echo $ab_amy_settings['yt-bg-repeat'];?> }); 
        });
        </script><?php 
	
}?>

<?php if($ab_amy_settings['yt-bg-cotrols'] == true  && $ab_amy_settings['yt-active'] == true ){?>
<script>
//Youtube controls
//==================================================
jQuery(document).ready(function($){
...
<?php	if( $ab_amy_settings['bg-active'] == true && $ab_amy_bgslideshow == 1){ ?>   
<script>	

		jQuery(document).ready(function($){
			'strict mode';
			if(window.hasownbg != 1){
				<?php if(isset($ab_amy_settings['bg-slides'][1]['image'])){?>
				jQuery.vegas('slideshow', {
				delay:<?php echo $ab_amy_settings['bg-delay']; ?>,
				backgrounds:[
				<?php $i = 0; 
					foreach( $ab_amy_settings['bg-slides'] as $slide ){?>
					
					{ src:'<?php echo $ab_amy_settings['bg-slides'][$i]['image'];?>', fade:<?php echo $ab_amy_settings['bg-fade']; ?>, valign:'<?php echo $ab_amy_settings['bg-vposition']; ?>', align:'<?php echo $ab_amy_settings['bg-hposition']; ?>' },
			 ...
			 <?php }else{?>
				...
				jQuery.vegas({
						src:'<?php echo $ab_amy_settings['bg-slides'][0]['image'];?>', 
						fade:<?php echo $ab_amy_settings['bg-fade']; ?>, 
...
//Display wellcome buble (use cookie to show only once)
	...
	function initInstructions() {
		...
		<?php if($ab_amy_settings['welcome-msg'] == '1'){ ?>
