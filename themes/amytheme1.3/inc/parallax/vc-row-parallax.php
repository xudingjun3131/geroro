...
if ( ! class_exists( 'GambitVCParallaxBackgrounds' ) ) {
	...
	class GambitVCParallaxBackgrounds {
			...
			jQuery(document).ready(function($) {
				...
				$('div.bg-parallax').each(function() {
					...
					$nextRows.each(function() {
						...
						$(this).prev().addClass('bg-parallax-parent ')
						// we need to clear the row background to make the parallax visible
						.css( {
							'overflow':'visible',
						} );
		...
		//		wp_register_script('ab_tf_jquery-scrolly', get_template_directory_uri().'/functions/parallax/jquery.scrolly.js', false,'1.0',true);
			...
			if (! empty($gmbt_prlx_layerslider)){ 
				$parallaxClass = "bg-parallax isprlx";
			}
            ...
            if ( $type == 'video' ) {
                if ( ! empty( $gmbt_prlx_video_youtube ) ) {
                    $videoDiv = "<div id='video-" . rand( 10000,99999 ) . "' data-youtube-video-id='" . $gmbt_prlx_video_youtube . "' data-mute='" . ( $gmbt_prlx_video_youtube_mute == 'mute' ? 'true' : 'false' ) . "' data-height-correction='" . $gmbt_prlx_video_height_correction . "' data-width-correction='" . $gmbt_prlx_video_width_correction . "'><div id='video-" . rand( 10000,99999 ) . "-inner'></div></div>";
